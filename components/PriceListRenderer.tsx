...
const PriceListRenderer: React.FC<Props> = ({ data }) => {
  ...
  const CompactTable = ({ category, className = "" }: { category?: PriceCategory, className?: string }) => {
    ...
    return (
      <div className={`mb-2 ${className}`}>
        <div className="bg-[#047857] text-white px-3 py-1 text-[13px] font-black rounded-t-sm shadow-sm flex justify-between items-center border-b border-[#065f46]">
          {renderHeader(category.title)}
        </div>
        <div className="border-2 border-[#047857] bg-white overflow-hidden rounded-b-sm shadow-sm">
          {category.items.map((item, idx) => (
            <div key={idx} className={`flex justify-between px-3 py-1 text-[11.5px] border-b border-[#f0fdf4] last:border-0 ${idx % 2 === 0 ? 'bg-white' : 'bg-[#f0fdf4]'}`}>
  ...
  return (
    <div className="flex flex-col gap-0 items-center py-8 print:py-0 bg-gray-300 print:bg-white print:block min-h-screen print:min-h-0">
      ...
      <div className="responsive-a4-wrapper">
        <div className="a4-page p-6 flex flex-col text-gray-800">
          <h2 className="text-[#065f46] border-b-[4px] border-[#065f46] mb-3 font-black text-2xl italic tracking-[0.2em] pb-1.5 uppercase">自費歯冠修復料金一覧</h2>
          ...
          <div className="mt-auto text-right text-[10px] text-gray-400 font-bold italic">※材料代別の項目は、別途貴金属等の費用が発生します。</div>
        </div>
      </div>
