...
const PriceListRenderer: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div className="flex flex-col gap-0 items-center py-0 print:block w-full">
      ...
      {/* PAGE 1: FRONT COVER & IMPLANT - Balanced Spacing, NO Black Bar */}
      <div className="responsive-a4-wrapper">
        <div className="a4-page p-12 flex flex-col h-full bg-white">
          <div className="w-full text-right text-[12px] text-gray-400 mb-12 font-black">発行日：{formattedDate}</div>
          
          <div className="w-full text-center border-y-[3px] border-gray-800 py-12 mb-10">
            <h1 className={`text-3xl font-black mb-4 ${!data.clinic.name ? 'text-gray-200 italic' : 'text-gray-800'}`}>
          ...
          <div className="w-full flex justify-end mb-12">
            <div className="border-[2px] border-gray-400 px-8 py-2.5 text-lg bg-white shadow-sm font-bold flex items-center gap-4">
          ...
          <div className="w-full flex-1">
             {implantCat && (
               <div className="mb-4">
                 <div className="bg-[#ea580c] text-white px-8 py-3 text-[16px] font-black flex justify-between items-center rounded-t shadow-md">
                 ...
                 <div className="border-[3px] border-[#ea580c] bg-white overflow-hidden rounded-b shadow-lg">
                   {implantCat.items.map((item, idx) => (
                     <div key={idx} className={`flex justify-between px-8 py-2 text-[11.5px] border-b border-[#fff7ed] last:border-0 ${idx % 2 === 0 ? 'bg-white' : 'bg-[#fff7ed]'}`}>
                       <span className="flex-1 font-bold text-gray-800 leading-tight py-0.5">{sanitizeName(item.name)}</span>
                       <span className="w-40 text-right font-black text-[#9a3412] leading-tight text-[12.5px] py-0.5">¥ {item.price}</span>
                     </div>
                   ))}
                 </div>
          ...
          {/* Footer Area - NO BLACK BAR to avoid cut off */}
          <div className="w-full mt-auto pt-8 pb-4 border-t-[2px] border-gray-100">
             ...
             <div className="text-center text-[9px] text-gray-600 font-bold mb-3 leading-relaxed">
      ...
      {/* PAGE 2: Insurance Prices - ABSOLUTELY UNCHANGED */}
      <div className="responsive-a4-wrapper">
      ...
      {/* PAGE 3: Private Dentures - CORRECTED & SPAN OUT BALANCED */}
      <div className="responsive-a4-wrapper">
        <div className="a4-page p-10 flex flex-col text-gray-800 bg-white">
          <h2 className="text-[#ea580c] border-b-[5px] border-[#ea580c] mb-10 font-black text-2xl italic tracking-[0.3em] pb-2 uppercase">自費義歯料金一覧</h2>
          
          <div className="flex flex-row gap-6 mb-10">
            <div className="w-1/2">
              <div className="bg-[#ea580c] text-white px-4 py-1.5 text-[13px] font-black shadow-sm rounded-t">自費義歯排列料</div>
              <div className="border-[2px] border-[#ea580c] bg-white rounded-b overflow-hidden shadow-sm">
                {privateGishiBasic?.items.slice(0, 5).map((item, i) => (
                  <div key={i} className={`flex justify-between px-3 py-2.5 text-[11px] border-b border-[#fff7ed] last:border-0 font-bold ${i % 2 !== 0 ? 'bg-[#fff7ed]' : ''}`}>
            ...
            <div className="w-1/2">
              <div className="bg-[#ea580c] text-white px-4 py-1.5 text-[13px] font-black shadow-sm rounded-t">自費義歯完成料</div>
              <div className="border-[2px] border-[#ea580c] bg-white rounded-b overflow-hidden shadow-sm">
          ...
          <div className="mb-10">
            <div className="bg-[#ea580c] text-white px-4 py-1.5 text-[13px] font-black shadow-sm rounded-t">{privateGishiNonclasp?.title}</div>
            <div className="flex flex-row gap-4 border-[2px] border-[#ea580c] bg-white p-4 rounded-b shadow-sm">
              <div className="w-1/2">
                <div className="text-[10px] font-black text-orange-800 border-b-2 border-orange-50 mb-2 px-1 uppercase tracking-tighter">エステショット / バイオプラスト</div>
                {privateGishiNonclasp?.items.slice(0, 6).map((item, i) => (
                  <div key={i} className={`flex justify-between px-2 py-2 text-[10.5px] font-black ${i % 2 !== 0 ? 'text-orange-700 italic pl-5' : 'text-gray-800 border-b border-[#fff7ed]'}`}>
              ...
              <div className="w-1/2 border-l-2 border-orange-50 pl-5">
                <div className="text-[10px] font-black text-orange-800 border-b-2 border-orange-50 mb-2 px-1 uppercase tracking-tighter">TUM</div>
          ...
          <div className="mb-10">
            <div className="bg-[#ea580c] text-white px-4 py-1.5 text-[13px] font-black shadow-sm rounded-t">金属床 (コバルト / チタン / 貴金属)</div>
            <div className="border-[2px] border-[#ea580c] bg-white rounded-b overflow-hidden shadow-sm">
               <table className="w-full text-[10.5px] border-collapse">
                  <thead className="bg-[#ffedd5] text-[#9a3412] font-black border-b border-[#fed7aa]">
                    <tr><th className="px-5 py-3 text-left">内容</th><th className="w-24">コバルト</th><th className="w-24">チタン</th><th className="w-24">貴金属</th></tr>
                  </thead>
                  <tbody className="bg-white font-black text-right">
                    {privateGishiMetal?.items.map((item, i) => (
                      <tr key={i} className={i % 2 !== 0 ? "bg-[#fff7ed]" : "border-b border-[#ffedd5]"}>
                        <td className="text-left px-5 py-3 text-orange-900">{sanitizeName(item.name)}</td>
          ...
          <div className="mb-10">
            <div className="bg-[#ea580c] text-white px-4 py-1.5 text-[13px] font-black shadow-sm rounded-t">{privateGishiOptions?.title}</div>
            <div className="border-[2px] border-[#ea580c] bg-white p-4 rounded-b shadow-sm">
              <div className="flex flex-row gap-6">
                <div className="w-1/2">
                  <table className="w-full text-[9.5px] border-collapse font-black">
                    <thead className="bg-[#ffedd5] text-[#9a3412] border-b border-[#fed7aa]">
                      <tr><th className="px-2 py-2 text-left">内容</th><th className="w-10">Co</th><th className="w-10">Ti</th><th className="w-10">貴</th></tr>
                    </thead>
                    <tbody className="text-right">
                      {privateGishiOptions?.items.slice(0, 6).map((item, i) => (
                        <tr key={i} className="border-b border-[#fff7ed]"><td className="text-left px-2 py-2 truncate">{sanitizeName(item.name)}</td><td>{parseMultiPrice(item.price, 0)}</td><td>{parseMultiPrice(item.price, 1)}</td><td>{parseMultiPrice(item.price, 2)}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
                <div className="w-1/2 border-l-2 border-orange-50 pl-6">
                  <table className="w-full text-[9.5px] border-collapse font-black">
                    ...
                    <tbody className="text-right">
                      ...
                      <tr className="font-bold text-orange-900 bg-[#fff7ed]"><td className="text-left px-2 py-2">自費：トレー / 咬合床</td><td colSpan={3} className="text-center font-black">各 ¥2,500</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
          ...
          {/* BOTTOM SECTION - Restored separation of Silicon Lining and Others */}
          <div className="mt-auto pt-4">
            <div className="flex flex-row gap-8">
               <div className="w-1/2">
                 <div className="bg-[#ea580c] text-white px-4 py-1.5 text-[12.5px] font-black shadow-sm rounded-t">シリコン裏装加工</div>
                 <div className="border-[2px] border-[#ea580c] bg-white p-4 rounded-b text-[10.5px] space-y-2 font-black shadow-sm">
               ...
               <div className="w-1/2">
                 ...
                 <div className="border-[2px] border-[#ea580c] bg-white p-4 rounded-b text-[10.5px] space-y-2 font-black shadow-sm flex flex-col min-h-[148px]">
                   ...
                   <div className="mt-auto text-[8.5px] text-[#9a3412] italic pt-4 leading-tight border-t border-orange-50">
                     ※貴金属は材料代別途となります。<br/>
      ...
      {/* PAGE 4: Private Crowns - ABSOLUTELY UNCHANGED */}
      <div className="responsive-a4-wrapper">
